...
function read_data(βs, prefix, ix)
    ...
    for β in βs
        @load "J1J2/deprecated-check-again/$(prefix)$(β).jld2" fs Es vars
        S = (Es[end] - fs[end]) * β
...
free_energies, enegies, variances, entropies = read_data(βs, "dimer_phase_beta", 1);
...
scatter!(ax1, 1 ./ βs, free_energies, label=L"\text{cMPO, }χ=6")
...
scatter!(ax2, 1 ./ βs[1:end], entropies, label=L"\text{cMPO, }χ=6")
...
scatter!(ax3, 1 ./ βs[1:end], variances, label=L"\text{cMPO, }χ=6")
scatter!(ax3, 1 ./ βs[1:end], variances2, label=L"\text{cMPO, }χ=9")
axislegend(ax3, position=:rt, framevisible=false)
@show fig

save("J1J2/J1J2_dimer_phase_result_deprecated.pdf", fig)
