...
function read_data(βs, prefix)
    ...
    chis = Int[]
    for β in βs
        @load "J1J2/gauged_cooling/data/$(prefix)$(β).jld2" f E var ψ
        ψL = Wmat * ψ
        S = (E - f) * β
        χ = dim(space(ψ))
...
free_energies, enegies, variances, entropies, chis, EEs, EEsyss = read_data(βs, "dimer_phase_beta");
...
scatter!(ax1, 1 ./ βs[end-11:end], free_energies[end-11:end], label=L"\text{cMPO}")
...
scatter!(ax2, 1 ./ βs[1:end][end-11:end], entropies[end-11:end], label=L"\text{cMPO}")
...
scatter!(ax3, 1 ./ βs[1:end][end-11:end], variances[end-11:end], label=L"\text{cMPO}")
...
scatter!(ax4, 1 ./ βs[1:end][end-11:end], chis[end-11:end], label=L"\text{cMPO}")
...
scatter!(ax5, 1 ./ βs[1:end][end-11:end], EEs[end-11:end], label=L"\text{cMPO}")
...
scatter!(ax6, 1 ./ βs[1:end][end-11:end], EEsyss[end-11:end], label=L"\text{cMPO}")
axislegend(ax5, position=:rb, framevisible=false)
@show fig

save("J1J2/gauged_cooling/J1J2_dimer_phase_result_cooling.pdf", fig)
