...
for β in βs
    global ψ
    ψ, f, E, var = power_iteration(T, Wmat, β, ψ, PowerMethod(tol_ES=1e-9))
    @save "heisenberg_FMXY/data/heisenberg_hz$(hz)_beta$(β)-toles10.jld2" β f E var ψ
    @show β, f, E, var
end

β = βs[end-15]
@load "heisenberg_FMXY/data/heisenberg_hz$(hz)_beta$(β)-toles10.jld2" β f E var ψ
